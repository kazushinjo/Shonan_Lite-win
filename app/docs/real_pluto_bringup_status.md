# Pi5実機PlutoSDRブリングアップ 調査報告書

作成日: 2026-08-09
対象: `pi5/rx/shonan_rx.py`(gr-dvbs2rx RX)+ `pi5/src/tx_main.cpp`(aff3ct TX)、
...
一旦棚上げし、Pi5のCPU実時間マージン確認など他の項目に移ることだと考えられる。**
...
- `pi5/gui/backend.py`: `TxController`を全面書き換え。SSH関連コード(sshpass、
  ...
  Pluto側のpluto_dvbライフサイクルは`rtmppluto.sh`が自律管理するため、Pi5側から
  リモートプロセスを気にする必要が一切なくなった。
  - ステータス判定: RTMP方式ではPluto側のUnderflow等がPi5から見えなくなる
  ...
  - カメラ+音声診断のビットレートを800k/64k→symbol_rate_hzに比例した値
    (既定1.9Mbps程度)に引き上げ。
- `pi5/gui/screens/testequipment.py`/`tx.py`: `underflow_count`を参照する箇所を
...
`pluto_dvb`を使っており、Pi5版のaff3ct実装(未解決のBBHEADER CRC8不具合を抱えたまま)
の方が他の兄弟プロジェクトとは異なる独自路線だった。旧`shonan_tx`(aff3ct、
`pi5/src/tx_main.cpp`)・`run_tx.sh`はソースを削除せずGUIからの呼び出しのみ停止した
(手動CLI利用や将来の参照用に残す)。**

### 変更内容

- `pi5/gui/backend.py`: `TxController.start()`/`start_camera_audio()`を
  ...
  カウント)ベースに変更。
- `pi5/gui/settings_store.py`: `bandwidth_hz`/`rf_port`/`runtime_dir`は
...
- `pi5/gui/screens/testequipment.py`/`tx.py`: TX単体診断・カメラ+音声診断の
  健全性判定を`connected`かつ累積`underflow_count`が増えていないことに変更。
- `pi5/rx/shonan_rx.py`: CLI既定値を`pluto_dvb`向け(`--framesize normal`
...
`ffmpeg | ssh ... pluto_dvb`をPi5上でbashスクリプトとして直接実行すると
...
- Pi5の`shonan-gui.service`は**通常稼働に復元済み**(タッチGUIが使える状態)。
...
#    ffmpegはPi5側で実行し、パイプの先でPlutoへSSHログイン(root/analog)して
...
# 4. 別ターミナル(またはPi5上の別セッション)でRX起動
python3 ~/shonan-pi5/pi5/rx/shonan_rx.py --pluto-uri ip:192.168.2.1 \
...
LAN経由(192.168.0.136、Pi5と同一eth0セグメント)に変更された。
...
**対処(コミット済み)**: `pi5/rx/shonan_rx.py`のバッファサイズを`0x8000`→
...
- `~/gr-dvbs2rx`の`pl_frame_sync.cc`は`[unlockdbg]`にt_ms(絶対時刻)を追加した
  ...
  削除すること。`pi5/docs/patches/gr-dvbs2rx_pi5_bringup.patch`は
  未更新なので、次回セッションでこの追加分も含めてエクスポートし直すこと)。
- `pi5/rx/shonan_rx.py`のIIOバッファサイズ拡大(`0x40000`)はリポジトリに
...
- Pi5 + PlutoSDR無印(Pluto+ではない)、F5OEO製DATVカスタムファームウェア(`datvplutofrm` v0.32-dirty)
...
ffmpeg -f v4l2 -i /dev/video0 -c:v libx264 -preset ultrafast -tune zerolatency \
  ...
  /home/pi/shonan-pi5/pi5/scripts/run_tx.sh --pluto-uri ip:192.168.2.1 \
...
根本修正として、同じユーザーの別プロジェクト(iOS版・同じ
...
`pi5/gui/backend.py`の`RxController`に、`frame`カウンタが8秒間進まなければ
...
1. **aff3ct側TXの非標準的な挙動**。`aff3ct`は元々研究・教育用ライブラリであり、
   ...
   (`~/shonan-pi5-native-build/.build-tmp-dvbs2/dvbs2/src/common/Module/Framer/`)。
...
- `lib/bbdescrambler_bb_impl.cc`: `[descdbg2]`(デスクランブル前の生バイト、
  48バイト分)
- `pi5/rx/shonan_rx.py`: `[flowdbg]`(ブロック間アイテム流量)、`[fecdbg]`
...
`pi5/docs/patches/gr-dvbs2rx_pi5_bringup.patch`にエクスポート済み
(`git -C ~/gr-dvbs2rx diff`の出力そのまま)。新しく`~/gr-dvbs2rx`をクローン
し直した場合は、このパッチを適用してから再ビルドすること:

```sh
cd ~/gr-dvbs2rx
git apply /home/pi/shonan-pi5/pi5/docs/patches/gr-dvbs2rx_pi5_bringup.patch
cd build
...
|---|---|---|
| 基準(SSH経由、ライブ映像) | `ffmpeg`(Pi5)→SSHパイプ→`pluto_dvb`(Pluto) | 42件 |
...
- 一時デバッグ計装の削除（既知の制約の追加調査時に必要となるため保留。`gr-dvbs2rx`側は
  `pi5/docs/patches/gr-dvbs2rx_pi5_bringup.patch`参照)
- Pi5のCPU性能の実時間マージン確認(理論最大フレームレート約90fpsに対し、
