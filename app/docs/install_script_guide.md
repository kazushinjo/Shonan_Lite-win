...
```sh
./app/scripts/install_ssh.sh      # SSHでclone/pull(private repoのため通常はこちら)
./app/scripts/install.sh          # HTTPSでclone/pull(既定。privateなので認証エラーになる)
```
...
GUI本体(`app/gui/main.py`)を動かすために必要な、Debianパッケージ一式を`apt`で
導入する。
...
`app/gui/main.py`は素の`InputPanel.qml`ではなく`app/gui/qml/
...
`app/gui/qml/InputPanelWrapper.qml`と`app/gui/main.py`のコード側の対応であり、
...
`app/rx/shonan_rx.py`は`from gnuradio import gr, analog, blocks, iio, dvbs2rx`を
...
`app/docs/patches/gr-dvbs2rx_pi5_bringup.patch`として適用する(既に適用済みなら
自動でスキップされる)。参考実装として`app/third_party/rpi-dvbs2-receiver-gui/`
...
- `app/docs/qtvirtualkeyboard_ja_build.md` — 3/6のビルド手順の一次情報、
  実機で遭遇したハマりどころの詳細
- `app/docs/patches/qtvirtualkeyboard_style_dark_language_popup.patch` —
  3/6で適用されるダークテーマパッチの実体(unified diff形式、`git diff`相当)
- `app/docs/shonan_pi5_operation_manual.docx` / `app/gui/manual_content.py` —
  GUIの操作方法そのもの(インストール後の使い方)
- `app/docs/patches/gr-dvbs2rx_pi5_bringup.patch` — 4/6で適用される受信安定化
  パッチの実体
- `app/third_party/rpi-dvbs2-receiver-gui/` — GNU Radio/gr-dvbs2rx受信フロー
