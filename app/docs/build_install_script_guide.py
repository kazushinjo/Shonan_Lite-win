...
r = p.add_run("Shonan_Lite for RasPI5")
r.bold = True
r.font.size = Pt(26)
...
add_para(
    "Raspberry Pi 5 + ADALM-Pluto+によるDVB-S2 DATV送受信タッチGUIシステム。"
    "本体はapp/配下(Python/PyQt5、eglfs直描画)。同系統の別プラットフォーム"
    "移植版(Android/iOS)は別リポジトリ(Shonan_Lite-android/Shonan_Lite-iPad等)"
    "で管理している。"
)

d.add_heading("0. Raspberry Pi OSのインストール", level=1)
add_para("Pi5本体に、あらかじめRaspberry Pi OSをインストールしておく。")
add_numbered([
    "PCでRaspberry Pi Imager(https://www.raspberrypi.com/software/)を起動する。",
    "「デバイスを選択」でRaspberry Pi 5を選ぶ。",
    ...
    "「書き込む」を実行し、完了後microSD/NVMeをPi5に取り付けて起動する。",
...
add_code_block([
    ...
    "cd Shonan_Lite-pi5",
    "./app/scripts/install.sh          # HTTPSでclone(既定)",
    "# ./app/scripts/install_ssh.sh    # GitHubにSSH鍵を登録済みならこちらでも可",
])
add_para(
...
add_code_block([
    "SKIP_JA_KEYBOARD=1 SKIP_GNURADIO_BUILD=1 ./app/scripts/install.sh",
])
add_para(
...
add_para(
    "詳細な各手順の解説・トラブルシュートはapp/docs/install_script_guide.mdを"
    "参照。"
)

d.add_heading("関連ドキュメント", level=1)
add_bullets([
    "app/docs/install_script_guide.md — install.shの詳細ガイド",
    "app/docs/qtvirtualkeyboard_ja_build.md — 日本語オンスクリーンキーボードの"
    "ビルド手順・ハマりどころ",
