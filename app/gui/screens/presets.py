...
_PRESET_FIELDS = (
    ...
    "overlay_callsign_font_size", "overlay_note_font_size",
...
class PresetsScreen(SettingsSubScreen):
    def __init__(self, main_window):
        ...
        note = QtWidgets.QLabel(
            tr("各設定画面の現在値を登録/変更で保存。選択時はPlutoにも反映します。\n"
               "送受信中は編集不可。RFループバックは TX → 40 dB以上 → RX。",
               "Save saves the current values of each settings screen. Selecting a preset also applies it to Pluto.\n"
               "Cannot be edited while TX/RX is running. RF loopback: TX -> 40 dB or more -> RX."))
    ...
    def _refresh(self):
        ...
        for index, button in enumerate(self._preset_buttons):
            ...
            if index == 0 and not item:
                label = tr("RFループバック", "RF Loopback") + "  437.000 MHz / 500 kS/s / QPSK / FEC 3/5"
            else:
                label = item.get('name', '') if item else tr('未登録', 'Empty')
            button.setText(f"{tr('プリセット', 'Preset ')}{index + 1}  {label}")
    ...
    def _save_current(self, index):
        if self._busy():
            error_dialog(self, tr("登録できません", "Cannot Save"),
                         tr("送受信を停止してから実行してください。", "Stop TX/RX first."))
            return
        presets = self.main_window.settings.presets
    ...
    def _select_or_register(self, index):
        ...
        if index >= len(presets) or not presets[index].get("values"):
            ...
            return
        if self._busy():
            error_dialog(self, tr("選択できません", "Cannot Select"),
                         tr("送受信を停止してから実行してください。", "Stop TX/RX first."))
            return
    ...
    def _delete_preset(self, index):
        if self._busy():
            error_dialog(self, tr("削除できません", "Cannot Delete"),
                         tr("送受信を停止してから実行してください。", "Stop TX/RX first."))
            return
        presets = self.main_window.settings.presets
        if index >= len(presets) or not presets[index].get("values"):
        ...
        answer = QtWidgets.QMessageBox.question(
            ...
            tr(f"「{presets[index].get('name', '')}」を削除しますか？",
               f"Delete \"{presets[index].get('name', '')}\"?"))
        ...
        try:
            ...
            error_dialog(self, tr("Pluto設定エラー", "Pluto Settings Error"),
                         tr(f"プリセットをPlutoへ反映できませんでした。\n{exc}",
                            f"Could not apply the preset to Pluto.\n{exc}"))
