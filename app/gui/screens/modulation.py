...
_MOCK_SCHEMES = ("BPSK", "QPSK", "8PSK", "16APSK")
...
class ConstellationWidget(QtWidgets.QWidget):
    ...
    def paintEvent(self, _event):
        ...
        point_radius = 10 if len(points) <= 16 else 8
        for x, y in points:
...
class ModulationScreen(SettingsSubScreen):
    def __init__(self, main_window):
        ...
        label.setStyleSheet("font-size: 29px; font-weight: bold; color: #54bce0;")
        ...
        for scheme in _MOCK_SCHEMES:
            ...
            button.setStyleSheet(
                ...
                " font-size: 29px; font-weight: bold; min-height: 44px; }"
            ...
            if scheme not in MODULATION_SCHEMES:
                ...
                button.setToolTip(tr("現在の送受信経路では未対応", "Not supported on the current TX/RX path"))
            else:
        ...
        bits_caption = QtWidgets.QLabel(tr("ビット/シンボル：", "Bits/symbol: "))
        bits_caption.setStyleSheet("font-size: 28px;")
        bottom.addWidget(bits_caption)
        self.bits_label = QtWidgets.QLabel()
        self.bits_label.setStyleSheet("font-size: 28px; font-weight: bold;")
        bottom.addWidget(self.bits_label)
        bottom.addStretch(1)
        self.scheme_label = QtWidgets.QLabel()
        self.scheme_label.setStyleSheet("font-size: 28px; font-weight: bold; color: #54bce0;")
