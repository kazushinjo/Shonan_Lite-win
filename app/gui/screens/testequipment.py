...
class TestEquipmentScreen(SettingsSubScreen):
    def __init__(self, main_window):
        ...
        self.result_table = QtWidgets.QTableWidget(3, 3)
        self.result_table.setHorizontalHeaderLabels([tr("項目", "Item"), tr("ステータス", "Status"), tr("結果", "Result")])
        ...
        self.result_table.horizontalHeader().setFixedHeight(36)
        ...
        self.result_table.setStyleSheet(
            "QTableWidget { background: #191d1f; color: white; border: none;"
            " gridline-color: #30383c; font-size: 24px; }"
            "QHeaderView::section { background: #252a2d; color: #cccccc; border: none;"
            " padding: 3px; font-size: 24px; font-weight: bold; }"
            "QTableWidget::item { padding: 3px; }"
        )
        for row, name in enumerate((tr("Pluto SDR接続", "Pluto SDR connection"), tr("送信テスト", "TX test"),
                                 tr("受信テスト", "RX test"))):
        ...
        self.run_btn.setMaximumHeight(44)
        self.run_btn.setFixedHeight(44)
        self.run_btn.setStyleSheet("font-size: 22px; font-weight: bold;")
        self.run_btn.clicked.connect(self._on_run)
        button_row.addWidget(self.run_btn, 1)
        self.camera_audio_run_btn = QtWidgets.QPushButton(tr("カメラ＋音声診断", "Camera + Audio Test"))
        self.camera_audio_run_btn.setMinimumHeight(0)
        self.camera_audio_run_btn.setMaximumHeight(40)
        self.camera_audio_run_btn.setFixedHeight(40)
        self.camera_audio_run_btn.setStyleSheet("font-size: 20px; font-weight: bold;")
        ...
        result_title = QtWidgets.QLabel(tr("診断結果", "Diagnostic Result"))
        result_title.setStyleSheet("font-size: 28px; font-weight: bold;")
        graph_layout.addWidget(result_title)
        self.health_label = QtWidgets.QLabel(tr("システム状態: 待機中", "System status: Idle"))
        self.health_label.setStyleSheet("font-size: 29px; font-weight: bold; color: #36b47a;")
        graph_layout.addWidget(self.health_label)
        self.log_view = QtWidgets.QTextEdit()
        self.log_view.setReadOnly(True)
        self.log_view.setStyleSheet("QTextEdit { background: #101416; color: #b9c4c8; border: 1px solid #30383c; border-radius: 6px; font-size: 22px; }")
    ...
    def _result_html(ok: bool) -> str:
        ...
        mark = "✓" if ok else "✗"
        text = tr("正常", "OK") if ok else tr("異常", "Error")
    ...
    def _on_run(self) -> None:
        ...
        if tx.is_running() or rx.is_running():
            self._log(tr("送受信中のため全体試験を開始できません", "Cannot start the full test while TX/RX is running"))
            return
        self._running = True
        ...
        self._log(tr(f"[診断] TX単体確認 ({TX_SECONDS}秒)", f"[Diag] TX check ({TX_SECONDS} s)"))
    ...
    def _finish_tx_phase(self) -> None:
        ...
        self._log(tr(f"[診断] TX結果: {self._ok_ng_html(tx_pass)} "
    ...
    def _start_rx_phase(self) -> None:
        ...
        self._log(tr(f"[診断] RX単体確認 ({RX_SECONDS}秒)", f"[Diag] RX check ({RX_SECONDS} s)"))
        self._set_row(2, tr("試験中", "Testing"), "…")
        rx.start(self.main_window.settings)
        self.status_label.setText(tr(f"RX継続確認中... (0/{RX_SECONDS}秒)", f"Checking RX... (0/{RX_SECONDS} s)"))
    ...
    def _rx_tick_wait(self) -> None:
        ...
        self.status_label.setText(tr(f"RX継続確認中... ({self._rx_tick}/{RX_SECONDS}秒)",
                                     f"Checking RX... ({self._rx_tick}/{RX_SECONDS} s)"))
    ...
    def _on_run_camera_audio(self) -> None:
        ...
        rx = self.main_window.rx_controller
        if tx.is_running() or rx.is_running():
            self._log(tr("送受信中のため診断を開始できません", "Cannot start the diagnostic while TX/RX is running"))
            return
    ...
    def _ca_warmup_done(self) -> None:
        self.status_label.setText(tr(
            f"カメラ＋音声送出確認中... (0/{CAMERA_AUDIO_SECONDS}秒)",
    ...
    def _ca_tick_check(self) -> None:
        ...
        self.status_label.setText(
            tr(f"カメラ＋音声送出確認中... ({self._ca_tick}/{CAMERA_AUDIO_SECONDS}秒) "
    ...
    def _finish_camera_audio(self) -> None:
        ...
        self._log(tr(f"[診断] キャプチャ結果: {self._ok_ng_html(capture_pass)} "
        ...
        self._log(tr(f"[診断] TX結果: {self._ok_ng_html(tx_pass)} "
                     ...
                     f"[Diag] TX result: {self._ok_ng_html(tx_pass)} "
                     f"(connected={self._ca_connected}, valid samples={self._ca_active}/{self._ca_checked})"))

        result = tr(f"カメラ＋音声送出: {self._result_html(overall_pass)}",
                    f"Camera + audio TX: {self._result_html(overall_pass)}")
