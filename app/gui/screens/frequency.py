...
_CHIP_STYLE = (
    ...
    " border-radius: 8px; padding: 2px 8px; font-size: 29px; font-weight: bold;"
...
class FrequencyScreen(SettingsSubScreen):
    def __init__(self, main_window):
        ...
        input_title.setStyleSheet("font-size: 29px; font-weight: bold; color: #cccccc;")
        ...
        self.freq_edit.setStyleSheet(
            ...
            " border-radius: 8px; font-size: 29px; font-weight: bold; padding: 6px 12px;")
        self.freq_edit.setMinimumHeight(48)
        ...
        unit_label.setStyleSheet("font-size: 28px; color: #9aa0a6; font-weight: bold;")
        input_layout.addWidget(unit_label)

        # グリッド自体がカードの余白を取り込んで行を拡大しないよう、
        # テンキー領域も固定高にする。
        keypad_container = QtWidgets.QWidget()
        keypad_container.setFixedSize(298, 218)
        ...
        for label, row, column in keypad_positions:
            ...
            button.setStyleSheet(
                ...
                " min-width: 70px; max-width: 70px; min-height: 50px;"
        ...
        self.current_label.setStyleSheet("font-size: 28px; color: #9aa0a6;")
        input_layout.addWidget(self.current_label)
        columns.addWidget(input_card, 4)

        # --- 右カラム: バンド選択(選択肢が多いため入力側より広めに確保する) ---
        band_card = QtWidgets.QFrame()
        band_card.setStyleSheet(
            "QFrame { background-color: #191d1f; border-radius: 12px; }"
            "QLabel { color: white; background: transparent; }")
        band_layout = QtWidgets.QVBoxLayout(band_card)
        band_layout.setContentsMargins(14, 8, 14, 8)
        band_layout.setSpacing(4)
        band_title = QtWidgets.QLabel(tr("バンド選択", "Band Selection"))
        band_title.setStyleSheet("font-size: 29px; font-weight: bold; color: #cccccc;")
        ...
        for band, info in BAND_PROFILES.items():
            ...
            chip = QtWidgets.QPushButton(f"{info['label_en'] if is_english() else info['label_ja']}\n{mhz:g} MHz")
