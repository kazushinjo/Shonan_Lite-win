...
class RssiGraphWidget(QtWidgets.QWidget):
    ...
    def paintEvent(self, _event) -> None:
        ...
        plot_rect = self.rect().adjusted(60, 14, -14, -34)
        ...
        painter.drawText(
            QtCore.QRectF(0, plot_rect.top() - 10, 54, 20),
            QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter, f"{y_min:.0f}")
        painter.drawText(
            QtCore.QRectF(0, plot_rect.bottom() - 10, 54, 20),
            QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter, f"{y_max:.0f}")
        # ★drawTextは枠からはみ出した分を切り取るため、枠幅を固定(90px)にすると
        # 「1283000 kHz」の先頭の「1」が欠けて「283000 kHz」に見えていた(実機で確認)。
        # 枠幅は文字列の実測幅から決める。
        fm = painter.fontMetrics()
        start_text = f"{start_hz / 1000:.0f}"
        end_text = f"{end_hz / 1000:.0f} kHz"
        start_w = fm.horizontalAdvance(start_text) + 8
        end_w = fm.horizontalAdvance(end_text) + 8
        painter.drawText(
            QtCore.QRectF(plot_rect.left() - 30, plot_rect.bottom() + 6, start_w, 24),
            QtCore.Qt.AlignLeft, start_text)
        painter.drawText(
            QtCore.QRectF(plot_rect.right() - end_w, plot_rect.bottom() + 6, end_w, 24),
...
class RssiScreen(SettingsSubScreen):
    ...
    def __init__(self, main_window):
        ...
        # ★Pi5実機(800x480)向けにfixedWidth(460)で調整されていたが、Windowsの
        # フォント描画では「周波数±5MHz」等のプリセットボタンの文字が右端で
        # 切れる(rx.py/tx.pyのステータスカードと同じ問題)。setMinimumWidthにして
        # 実際のラベル幅に応じて広がれるようにする。
        left.setMinimumWidth(460)
        left.setStyleSheet("QFrame { background: #191d1f; border: 1px solid #34434b; border-radius: 10px; } QLabel { font-size: 28px; }")
        ...
        self.center_freq_label.setStyleSheet("color: #54bce0; font-size: 22px; font-weight: bold;")
        ...
        for offset_mhz in (5, 10, 20):
            preset_btn = QtWidgets.QPushButton(tr(f"周波数±{offset_mhz}MHz", f"±{offset_mhz} MHz"))
        ...
        keypad_grid.setHorizontalSpacing(8)
        ...
        self.search_btn.setStyleSheet(
            ...
            " border-radius: 6px; font-size: 29px; font-weight: bold; }")
        ...
        self.best_freq_label.setStyleSheet("color: #eeeeee; font-size: 24px; font-weight: bold;")
        ...
        for button in (self.repeat_btn, self.once_btn):
            ...
            button.setMinimumSize(90, 44)
            button.setStyleSheet(
                "QPushButton { background: #303538; color: white; border: none;"
                " border-radius: 6px; font-size: 24px; font-weight: bold; padding: 4px 12px; }"
        ...
        gain_title.setStyleSheet("color: #eeeeee; font-size: 24px; font-weight: bold;")
        gain_row.addWidget(gain_title)
        gain_row.addStretch(1)
        gain_button_style = (
            "QPushButton { background: #0c91b5; color: white; border: none;"
            " border-radius: 6px; font-size: 26px; font-weight: bold; padding: 4px 12px; }"
            "QPushButton:pressed { background: #3f96b3; }"
            "QPushButton:disabled { background: #303538; color: #6b7880; }")
        self.agc_btn = QtWidgets.QPushButton("AGC")
        self.agc_btn.setCheckable(True)
        self.agc_btn.setMinimumHeight(44)
        self.agc_btn.setStyleSheet(
            "QPushButton { background: #303538; color: white; border: none;"
            " border-radius: 6px; font-size: 22px; font-weight: bold; padding: 4px 10px; }"
        ...
        self.gain_label.setMinimumWidth(90)
        self.gain_label.setStyleSheet("color: #eeeeee; font-size: 24px; font-weight: bold;")
    ...
    def on_show(self) -> None:
        ...
        # 自動的に取り込む(既定は±10MHz。他の幅が必要ならプリセットボタンで
        # 切り替え直す)。未設定の場合は既存の範囲欄をそのまま残す(警告は出さない
    ...
    def _update_center_label(self) -> None:
        lo_hz = self.main_window.settings.effective_lo_hz()
        if lo_hz is None:
            self.center_freq_label.setText(tr("中心周波数: 未設定(周波数画面で設定してください)",
                                              "Center frequency: Not set (set it on the Frequency screen)"))
        else:
            self.center_freq_label.setText(tr(f"中心周波数: {lo_hz / 1000:.0f} kHz",
                                              f"Center frequency: {lo_hz / 1000:.0f} kHz"))
    ...
    def _scan_tx_settings(self):
        ...
        繰り返すだけになり、何も送信されずRSSIが変化しなかった(Pi5実機で確認)。
    ...
    def _start_timer_if_still_scanning(self) -> None:
        ...
        # RSSI測定を始めると無意味な値を測定し続けてしまうため、その場合は
        # スキャン自体を中止する。
    ...
    def _commit_scan_result(self) -> None:
        ...
        self.best_freq_label.setText(
            tr(f"最も強い周波数: {self._best_freq / 1000:.0f} kHz (RSSI {self._best_rssi:g})",
