...
class StreamOutputScreen(SettingsSubScreen):
    ...
    def __init__(self, main_window):
        ...
        title.setStyleSheet("font-size: 29px; font-weight: bold; color: #cccccc;")
        left_layout.addWidget(title)
        protocol = QtWidgets.QComboBox()
        protocol.addItem("UDP")
        protocol.setFixedHeight(44)
        protocol.setStyleSheet("QComboBox { min-height: 44px; max-height: 44px; padding: 2px 8px; font-size: 29px; }")
        dest_label = QtWidgets.QLabel(tr("出力先", "Destination"))
        dest_label.setStyleSheet("font-size: 29px;")
        left_layout.addWidget(dest_label)
        left_layout.addWidget(protocol)
        pluto_uri_label = QtWidgets.QLabel("Pluto URI")
        pluto_uri_label.setStyleSheet("font-size: 29px;")
        left_layout.addWidget(pluto_uri_label)
        pluto_uri_row = QtWidgets.QHBoxLayout()
        pluto_uri_row.setSpacing(8)
        self.pluto_uri_edit = QtWidgets.QLineEdit()
        self.pluto_uri_edit.setFixedHeight(44)
        self.pluto_uri_edit.setStyleSheet(
            "QLineEdit { min-height: 44px; max-height: 44px; padding: 3px 8px; font-size: 29px; }")
        ...
        self.pluto_search_btn.setFixedHeight(44)
        self.pluto_search_btn.setFixedWidth(150)
        self.pluto_search_btn.setStyleSheet(
            "QPushButton { min-height: 44px; max-height: 44px; font-size: 29px; padding: 3px 8px; }")
        ...
        self.pluto_discovery_status_label.setStyleSheet("color: #9aa0a6; font-size: 24px;")
        ...
        dest_port_label = QtWidgets.QLabel(tr("送信先ポート（Pluto側固定）", "Destination port (fixed on Pluto)"))
        dest_port_label.setStyleSheet("font-size: 29px;")
        left_layout.addWidget(dest_port_label)
        self.dest_port_edit = QtWidgets.QLineEdit(str(PLUTO_UDP_TS_PORT))
        self.dest_port_edit.setReadOnly(True)
        self.dest_port_edit.setFixedHeight(44)
        ...
        for key, label in (("protocol", tr("プロトコル", "Protocol")), ("ip", "Pluto URI"),
            ...
            name.setStyleSheet("color: #9aa0a6; font-size: 28px;")
            value = QtWidgets.QLabel()
            value.setAlignment(QtCore.Qt.AlignRight | QtCore.Qt.AlignVCenter)
            value.setStyleSheet("color: white; font-size: 29px;")
            row.addWidget(name)
        ...
        status.setStyleSheet("color: #9aa0a6; font-size: 28px;")
    ...
    def _add_int_field(self, key, label, parent_layout):
        ...
        edit.setStyleSheet("QLineEdit { min-height: 44px; max-height: 44px; padding: 3px 8px; font-size: 29px; }")
        ...
        field_label.setStyleSheet("font-size: 29px;")
    ...
    def on_show(self):
        ...
        self.info_labels["port"].setText(str(PLUTO_UDP_TS_PORT))
    ...
    def _search_pluto_ip(self) -> None:
        # ★android版StreamOutputSettingsScreen.kt(自動検出ボタン)と同じ判定基準・UXに
        # 合わせる: モーダルダイアログではなくボタン直下のインライン表示で進捗/結果を示す。
    ...
    def _on_pluto_discovery_finished(self, host: object) -> None:
        ...
        if host is None:
            self.pluto_discovery_status_label.setText(
                tr("Plutoが見つかりませんでした(電源・LAN接続を確認してください)",
