...
_CHIP_STYLE = (
    ...
    " border-radius: 8px; padding: 4px; font-size: 29px; font-weight: bold;"
    " min-height: 44px; }"
...
def _describe(rate: str) -> str:
    ...
    if overhead >= 40:
        return tr(
            ...
            "Prioritizes error correction. Demodulates reliably even at low C/N.")
    if overhead >= 15:
        return tr(
            "誤り訂正能力とスループットのバランスが取れた設定です。",
            "A balance between error correction and throughput.")
    return tr(
        "スループットを重視した設定です。良好なC/N環境で高い伝送効率を得られます。",
        "Prioritizes throughput. Achieves high transmission efficiency at good C/N.")
...
class FecScreen(SettingsSubScreen):
    def __init__(self, main_window):
        ...
        mode_title = QtWidgets.QLabel(tr("FECモード選択", "FEC Mode"))
        mode_title.setStyleSheet("font-size: 29px; font-weight: bold; color: #cccccc;")
        ...
        self.current_label.setStyleSheet("font-size: 29px; font-weight: bold;")
        info_layout.addWidget(self.current_label)

        self.description_label = QtWidgets.QLabel()
        self.description_label.setWordWrap(True)
        self.description_label.setStyleSheet("color: #cccccc; font-size: 28px;")
        info_layout.addWidget(self.description_label)

        self.overhead_label = QtWidgets.QLabel()
        self.overhead_label.setStyleSheet("color: #9aa0a6; font-size: 28px;")
        ...
        note = MemoNote(
            tr("変調方式(Modulation画面)と組み合わせてMod-Codを構成します。"
               ...
               "Mod-Cod is formed by combining this with the modulation (Modulation screen). "
               "Choosing an unsupported combination causes an error when TX/RX starts.")
        )
        note.setStyleSheet("color: #999999; font-size: 22px; padding: 4px;")
        info_layout.addWidget(note)
        columns.addWidget(info_card, 1)

        # --- 右カラム: フレーム構成(DATA/FEC/PARITY) ---
        frame_card = QtWidgets.QFrame()
        frame_card.setFixedWidth(180)
        ...
        frame_title = QtWidgets.QLabel(tr("フレーム構成", "Frame"))
        frame_title.setStyleSheet("font-size: 29px; font-weight: bold; color: #cccccc;")
    ...
    def _legend_dot(color: str, text: str) -> QtWidgets.QWidget:
        ...
        label.setStyleSheet("color: #cccccc; font-size: 28px;")
