# 山崎慎慈氏によるRaspberry Pi 5 DVB-S2受信システムの再捕捉安定化に関する技術報告　準備編
...
Raspberry Pi 5 + PlutoSDR による DVB-S2 受信ソフトウェア(`rpi-dvbs2-receiver-gui`)のビルド環境整備後、PlutoSDR の TX/RX を同軸ケーブルとアッテネータで直結するループバック構成により、受信パイプラインの一気通貫動作を確認した。
...
Raspberry Pi 5 + PlutoSDR 上に構築した DVB-S2 受信ビルド環境は、TX/RX ループバック試験において物理層ロックから TS 復調、UDP 経由での映像再生まで一貫して正常に動作することを確認した。
