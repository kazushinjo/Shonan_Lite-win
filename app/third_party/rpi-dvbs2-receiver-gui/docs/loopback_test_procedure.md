...
対象機: Raspberry Pi 5(ホスト名 `DVB-S2`, `192.168.0.135`, ユーザー `pi`)
...
解像度は160x120/10fps/400kbps程度に抑えること(320x240/25fps/1500kbpsだとPi5のCPUがソフトウェアエンコードに追いつかず実時間の0.3〜0.4倍程度まで落ち込む。160x120/10fpsなら実時間の1.2倍程度で安定)。

停止時も `pkill -f ffmpeg` を使わず、記録したPIDを個別に `kill` すること。

## 8. シンボルレート上限とHD映像伝送 【重要】

### 発見: RPi5-PlutoSDR間のUSBスループット実測上限
