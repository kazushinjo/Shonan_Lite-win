#!/usr/bin/env python3
"""shonan_rx -- Pi5ネイティブDVB-S2 RXフローグラフ。

GNU Radioフローグラフ構成をPythonで実装したもの。実証済みの動作方式である
gr-dvbs2rx(igorauad氏)+ gr-iioのfmcomms2_sourceを使う。
...
class _StdoutSink(gr.sync_block):
    """復調後TSを標準出力(バイナリ)へそのまま書き出す軽量シンクブロック。

    Windows版GUI(gui/backend.py RxController._start_windows_rx_pipeline)は
    mkfifo/setsidが無いため、FIFOではなく`QProcess.setStandardOutputProcess()`
    でこのプロセスの標準出力を表示用ffmpegの標準入力へ直結する
    (`shonan_rx.py | ffmpeg`をQt経由で行うのと同義)。--output-fifo同様、
    書き込み先(ffmpeg側)が先に終了するとBrokenPipeErrorになりうるため、
    その場合は例外を飲み込んでシンプルに以降の書き込みを止める。
    """

    def __init__(self):
        gr.sync_block.__init__(
            self, name="stdout_sink",
            in_sig=[gr.sizeof_char], out_sig=None)
    ...
    def work(self, input_items, output_items):
        ...
        if not self._closed:
            try:
                self._stdout.write(bytes(input_items[0]))
                self._stdout.flush()
            except (BrokenPipeError, OSError):
                self._closed = True
        return n

# 送信側と同じ信号をRXで受けるため、対応するconstellation/code_rateのみをサポートする。
...
def parse_args():
    p = argparse.ArgumentParser(description="shonan Pi5 DVB-S2 RX (gr-dvbs2rx)")
    ...
    # real_pluto_bringup_status.md参照。pluto_dvbはnormal
    ...
    output_group.add_argument("--output-fifo", help="復調後TSの出力先FIFOパス(Linux)")
    output_group.add_argument("--output-stdout", action="store_true",
                               help="復調後TSを標準出力へ書き出す(Windows。"
                                    "mkfifo/setsidが無いためQProcessでffmpegへ直結する)")
...
def main():
    ...
    if args.output_fifo is not None:
    ...
    if args.output_fifo is not None:
        ...
        # --output-stdout(Windows。gui/backend.py RxController.
        # _start_windows_rx_pipeline()がQProcess.setStandardOutputProcess()で
        # このプロセスの標準出力を表示用ffmpegの標準入力へ直結する)。
        # WindowsのCRTは既定でfd 1をテキストモードで開いており、そのままだと
        # MPEG-TSバイナリ中の0x0Aバイトが改行変換で破損するため、事前にバイナリ
        # モードへ切り替える必要がある。
